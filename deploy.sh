...
# Kiểm tra và cài đặt sfoundry nếu cần
if ! command -v scast &> /dev/null; then
    echo "scast chưa được cài đặt. Đang chạy sfoundryup để cài đặt Seismic Foundry..."
    sfoundryup
    if ! command -v scast &> /dev/null; then
        echo "Lỗi: Không thể cài đặt scast. Vui lòng kiểm tra sfoundryup hoặc liên hệ @lyronc trên Telegram."
        exit 1
    fi
else
    echo "scast đã được cài đặt."
fi
